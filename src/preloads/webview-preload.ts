...
const isolatedWorldMaps: Monarc.Preload.IsolatedWorldMaps = {};
const context: Monarc.Preload.Context = { Monarc: {} };

// Run the code with chrome and Monarc API integrated.
const runContentScript = (name, extensionId, isolatedWorldId, url, code) => {
  ...
  if (extension === undefined) {
    ...
    webFrame.executeJavaScriptInIsolatedWorld(isolatedWorldId, [{
      ...
      window.Monarc = context.Monarc;
      window.chrome = window.Monarc;
    });
  }
  webFrame.executeJavaScriptInIsolatedWorld(isolatedWorldId, [{
    code,
...
process.once('loaded', () => {
  if (document.location) {
    if (document.location.href.startsWith('Monarc://') ||
      (process.env.NODE_ENV === 'development' && document.location.href.startsWith('http://localhost:'))) {
      contextBridge.exposeInMainWorld('data', {
        about: ipcRenderer.sendSync('Monarc-scheme-loaded', document.location.href),
  ...
  ipcRenderer.on('Monarc-tabs-send-message', (event, message) => {
    ipcRenderer.send('Monarc-runtime-emit-on-message', message);
  });
  ipcRenderer.on('remove-Monarc-extension-result', (event, data): void => {
    if (data.result === 'OK') {
