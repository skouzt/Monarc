...
const register = (eventName: any, sess: Electron.Session, eventLispCaseName: string, id: number, digest: string, filter): void => {
  if ((eventName === 'onBeforeRequest') || (eventName === 'onBeforeSendHeaders')) {
    sess.webRequest[eventName](filter, (details, callback) => {
      ...
      ipcMain.once(`Monarc-web-request-${eventLispCaseName}-response-${digest}-${requestId}`, (event: Electron.Event, request) => {
        if (request) {
          if (request.cancel) {
      ...
      window.webContents.send('Monarc-web-request-intercepted', {
        eventLispCaseName,
        digest,
        requestId,
        details,
    ...
    sess.webRequest[eventName](filter, (details, callback) => {
      ...
      ipcMain.once(`Monarc-web-request-${eventLispCaseName}-response-${digest}-${requestId}`, (event: Electron.Event, response) => {
        if (response) {
          if (response.cancel) {
            callback({ cancel: true });
...
const registerWebRequestListeners = (): void => {
  const sess = session.fromPartition('persist:Monarc') as Electron.Session;
  ipcMain.on('Monarc-web-request-add-listener-on-before-request', (event: Electron.IpcMainEvent, extensionName, eventLispCaseName: string, digest: string, filter): void => {
    register('onBeforeRequest', sess, eventLispCaseName, event.sender.id, digest, filter);
  });
  ipcMain.on('Monarc-web-request-remove-listener-on-before-request', (event, extensionName, eventLispCaseName): void => {
    unregister('onBeforeRequest', sess);
  });
  ipcMain.on('Monarc-web-request-add-listener-on-before-send-headers', (event: Electron.IpcMainEvent, extensionName, eventLispCaseName: string, digest: string, filter): void => {
    register('onBeforeSendHeaders', sess, eventLispCaseName, event.sender.id, digest, filter);
  });
  ipcMain.on('Monarc-web-request-remove-listener-on-before-send-headers', (event, extensionName, eventLispCaseName): void => {
    unregister('onBeforeSendHeaders', sess);
  });
  ipcMain.on('Monarc-web-request-add-listener-on-send-headers', (event: Electron.IpcMainEvent, extensionName, eventLispCaseName: string, digest: string, filter): void => {
    register('onSendHeaders', sess, eventLispCaseName, event.sender.id, digest, filter);
  });
  ipcMain.on('Monarc-web-request-remove-listener-on-send-headers', (event, extensionName, eventLispCaseName): void => {
    unregister('onSendHeaders', sess);
  });
  ipcMain.on('Monarc-web-request-add-listener-on-headers-received', (event: Electron.IpcMainEvent, extensionName, eventLispCaseName: string, digest: string, filter): void => {
    register('onHeadersReceived', sess, eventLispCaseName, event.sender.id, digest, filter);
  });
  ipcMain.on('Monarc-web-request-remove-listener-on-headers-received', (event, extensionName, eventLispCaseName): void => {
    unregister('onHeadersReceived', sess);
  });
  ipcMain.on('Monarc-web-request-add-listener-on-response-started', (event: Electron.IpcMainEvent, extensionName, eventLispCaseName: string, digest: string, filter): void => {
    register('onResponseStarted', sess, eventLispCaseName, event.sender.id, digest, filter);
  });
  ipcMain.on('Monarc-web-request-remove-listener-on-response-started', (event, extensionName, eventLispCaseName): void => {
    unregister('onResponseStarted', sess);
  });
  ipcMain.on('Monarc-web-request-add-listener-on-before-redirect', (event: Electron.IpcMainEvent, extensionName, eventLispCaseName: string, digest: string, filter): void => {
    register('onBeforeRedirect', sess, eventLispCaseName, event.sender.id, digest, filter);
  });
  ipcMain.on('Monarc-web-request-remove-listener-on-before-redirect', (event, extensionName, eventLispCaseName): void => {
    unregister('onBeforeRedirect', sess);
  });
  ipcMain.on('Monarc-web-request-add-listener-on-completed', (event: Electron.IpcMainEvent, extensionName, eventLispCaseName: string, digest: string, filter): void => {
    register('onCompleted', sess, eventLispCaseName, event.sender.id, digest, filter);
  });
  ipcMain.on('Monarc-web-request-remove-listener-on-completed', (event, extensionName, eventLispCaseName): void => {
    unregister('onCompleted', sess);
  });
  ipcMain.on('Monarc-web-request-add-listener-on-error-occurred', (event: Electron.IpcMainEvent, extensionName, eventLispCaseName: string, digest: string, filter): void => {
    register('onErrorOccurred', sess, eventLispCaseName, event.sender.id, digest, filter);
  });
  ipcMain.on('Monarc-web-request-remove-listener-on-error-occurred', (event, extensionName, eventLispCaseName): void => {
...
const registerScheme = (scheme: string): void => {
  ...
  if (scheme === 'Monarc') {
    sess = session.fromPartition('persist:Monarc');
  }
  if (sess) {
  ...
  sess.setCertificateVerifyProc((request, callback) => {
    try {
      ...
      if (err.toString() === 'Error: Cannot read public key. OID is not RSA.') {
        console.error('(Monarc-browser) `node-forge` doesn\'t support ECC for now, so we fallback to the old method.');
        store.dispatch('updateCertificate', {
