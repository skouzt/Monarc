...
export default class BrowserMainView extends Vue {
  ...
  alarms: Monarc.BrowserMainView.AlarmArray = {};
  ...
  get window(): Monarc.Store.MonarcBrowserWindowProperty {
  ...
  getTabObject(tabIndex?: number): Monarc.Store.TabObject {
    let index: number | undefined = tabIndex;
    if (index === undefined) {
      if (this.currentTabIndex === undefined) {
        index = 0;
  ...
  lastOpenedTabs(): Monarc.Store.LastOpenedTabObject[] {
    const lastOpenedTabs: Monarc.Store.LastOpenedTabObject[] =
    ...
    lastOpenedTabs.forEach((lastOpenedTab) => {
      switch (lastOpenedTab.title) {
        case 'about:about':
          lastOpenedTab.title = this.$t('Monarc.aboutPage.title');
          break;
        case 'about:Monarc':
          lastOpenedTab.title = this.$t('Monarc.MonarcPage.title');
          break;
        case 'about:preferences/search':
          lastOpenedTab.title = this.$t('Monarc.preferencesPage.searchEngineProviderPage.title');
          break;
        case 'about:preferences/homepage':
          lastOpenedTab.title = this.$t('Monarc.preferencesPage.homePage.title');
          break;
        case 'about:preferences/pdfViewer':
          lastOpenedTab.title = this.$t('Monarc.preferencesPage.pdfViewerPage.title');
          break;
        case 'about:preferences/tab':
          lastOpenedTab.title = this.$t('Monarc.preferencesPage.tabConfigPage.title');
          break;
        case 'about:preferences/language':
          lastOpenedTab.title = this.$t('Monarc.preferencesPage.LanguagePage.title');
          break;
        case 'about:downloads':
          lastOpenedTab.title = this.$t('Monarc.downloadsPage.title');
          break;
        case 'about:history':
          lastOpenedTab.title = this.$t('Monarc.historyPage.title');
          break;
        case 'about:extensions':
          lastOpenedTab.title = this.$t('Monarc.extensionsPage.title');
          break;
        case 'about:newtab':
          lastOpenedTab.title = this.$t('Monarc.newtabPage.title');
          break;
        case 'about:playbooks':
          lastOpenedTab.title = this.$t('Monarc.playbooks.title');
          break;
        default:
          lastOpenedTab.title = this.$t('Monarc.aboutPage.title');
          break;
  ...
  // Monarc.alarms
  getAlarm(name: string): Monarc.BrowserMainView.Alarm | undefined {
    return this.alarms[name];
  }
  getAllAlarm(): Monarc.BrowserMainView.AlarmArray {
    return this.alarms;
  }
  ...
  createAlarm(name: string, alarmInfo: any): void {
    const alarm: Monarc.BrowserMainView.Alarm = {
      handler: null,
    };
    let timeout = 0;
  ...
  onNewWindow(event: Electron.NewWindowEvent, tabIndex: number): void {
    ...
    if (disposition === 'new-window') {
      ...
      if (options.width === 800 &&
        ...
        (event as any).newGuest = this.$electron.ipcRenderer.sendSync('new-Monarc-window', {
          url: event.url,
          follow: true,
        });
      }
  ...
  */
  ...
  onTabDuplicate(tabIndex: number): void {
    const tabObject: Monarc.Store.TabObject = this.getTabObject(tabIndex);
    if (tabObject) {
  ...
  onTabClick(tabIndex: number): void {
    ...
    const tabObject: Monarc.Store.TabObject = this.getTabObject(usedTabIndex);
    if (tabObject) {
          ...
          for (let i = current - 1; i >= 0; i -= 1) {
            try {
              ...
              if (match) {
                menuItems.push({
                  type: 'base64',
                  icon: constants.tabConfig.MonarcDefault.systemFavicon,
                  label: urls[i],
                  click: 'go-to-index',
                  index: i,
                });
  ...
  onCommonMenu(): void {
    ...
    if (currentWindow) {
      ...
      let sub: Electron.MenuItemConstructorOptions[] = [
        {
          label: this.$t('navbar.common.options.Monarc') as string,
          click: () => this.onNewTab(this.windowId, 'about:Monarc', false),
        },
      ];
          ...
          sub = sub.concat([
            {
              ...
              click: () => this.onNewTab(
                this.windowId,
                'https://github.com/MonarcProject/Monarc-browser/issues',
                true
              ),
            },
            {
              ...
              if (regMenuItem.type !== 'separator') {
                ...
                regMenuItem.click = (self, browserWindow) => {
                  this.$electron.remote.webContents.fromId(self.webContentsId)
                    .send(
                      `Monarc-context-menus-clicked-${self.extensionId}-${self.id}`,
                      params,
                  ...
                  submenu.forEach((sub) => {
                    ...
                    sub.click = (self, browserWindow) => {
                      this.$electron.remote.webContents.fromId(sub.webContentsId)
                        .send(
                          `Monarc-context-menus-clicked-${sub.extensionId}-${sub.id}`,
                          params,
                          this.getTabObject(this.currentTabIndex).id,
                          self,
                          browserWindow
  ...
  beforeMount(): void {
    ...
    if (!(process.env.NODE_ENV === 'test' && process.env.TEST_ENV === 'unit')) {
      ...
      ipc.once(
        ...
        (event: Electron.Event, suggestion: Monarc.Main.BrowserWindowSuggestionItem | null) => {
  ...
  mounted(): void {
    ...
    ipc.on('remove-non-bg-Monarc-extension', (event: Electron.Event, extensionId: string) => {
