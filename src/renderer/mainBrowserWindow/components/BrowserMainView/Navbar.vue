...
Vue.component('SuggestionItem', {
  ...
  render(h, ctx) {
    const suggestion: Monarc.Renderer.SuggestionObject = ctx.props.item;
          ...
          return h('div', { attrs: { class: 'url' } }, [
            ...
            [
              h('i', {
                attrs: {
                  class:
                    `el-icon-${ctx.parent.$store.getters.tabConfig.MonarcDefault.tabFavicon}`,
                },
              }),
            ]),
...
export default class Navbar extends Vue {
  ...
  suggestionItems: Monarc.Renderer.SuggestionItem[] = config.recommendTopSite;
  extensions: Monarc.API.ManifestObject[] = [];
  onbrowserActionClickedEvent: Event = new Event();
  onpageActionClickedEvent: Event = new Event();

  windowId: number;

  get dummyTabObject(): Monarc.Store.TabObject {
  ...
  get tabFavicon(): string {
    return this.$store.getters.tabConfig.MonarcDefault.tabFavicon;
  }
  ...
  get certificates(): Monarc.Store.Certificates {
  ...
  get url(): string {
    ...
    if (this.value === 'Monarc://about/#/newtab') {
      this.value = '';
  ...
  get currentSearchEngine(): Monarc.Store.SearchEngineObject {
    return this.$store.getters.currentSearchEngine;
  }
  get suggestionItemsByHistory(): any {
    const suggestionItems: Monarc.Renderer.SuggestionItem[] = [];
  ...
  get fancyContent(): string {
    if (this.tab.url === 'chrome://gpu/') {
      return '<div class="security-hint">Monarc://gpu</div>';
    }
    if (this.tab.url === 'Monarc://about/#/newtab') {
      return '<div class="security-hint">Monarc://about/newtab</div>';
    }
    if (this.tab.url.startsWith('Monarc://')) {
  ...
  @Watch('tab')
  onTab(newTab: Monarc.Store.TabObject): void {
  ...
  updateSecure(urlString: string): void {
    ...
    if (scheme === 'Monarc://' || scheme === 'Monarc-extension://' || scheme === 'about:') {
      this.secure = true;
      return;
    }
    if (scheme === 'https://') {
  ...
  unique(suggestions: Monarc.Renderer.SuggestionObject[]): Monarc.Renderer.SuggestionObject[] {
    const newSuggestions: Monarc.Renderer.SuggestionObject[] = [];
  ...
  onSelect(event: Monarc.Renderer.SuggestionObject): void {
    this.typing = false;
    if (event.item) {
  ...
  // eslint-disable-next-line max-len
  async querySearch(queryString: string, cb: (suggestions: Monarc.Renderer.SuggestionObject[]) => void): Promise<void> {
    const ipc = this.$electron.ipcRenderer;
    const currentSearchEngine: string = this.currentSearchEngine.name;
    const navbarSearch = this.$t('navbar.search');
    let suggestions: Monarc.Renderer.SuggestionObject[] = [];
    ...
    if (queryString) {
      if (queryString.startsWith('about:')) {
      ...
      } else if (queryString.startsWith('Monarc:')) {
        // Privileged Pages
        suggestions.unshift({
          item: {
            title: '',
        ...
        suggestions.unshift({
          item: {
            title: '',
            value: 'Monarc://preferences',
            url: 'Monarc://preferences',
            icon: 'document',
          },
        });
        suggestions.unshift({
          item: {
            title: '',
            value: 'Monarc://downloads',
            url: 'Monarc://downloads',
            icon: 'document',
          },
        });
        suggestions.unshift({
        ...
        suggestions.unshift({
          item: {
            title: '',
            value: 'Monarc://extensions',
            url: 'Monarc://extensions',
            icon: 'document',
          },
        });
        suggestions.unshift({
          item: {
            title: '',
            value: 'Monarc://playbooks',
            url: 'Monarc://playbooks',
            icon: 'document',
          },
        });
      }
    ...
    const entries: Monarc.Renderer.SuggestionObject[][] =
  ...
  extensionsMetadata(extensionId: string): Monarc.Store.ExtensionMetadata | null {
  ...
  setBrowserActionIcon(extensionId: string, iconInfo: Monarc.API.IconInfo): void {
    this.$nextTick(() => {
  ...
  setPageActionIcon(extensionId: string, iconInfo: Monarc.API.IconInfo): void {
    this.$nextTick(() => {
      this.$store.dispatch('updateExtensionMetadata', {
        extensionId,
        tabId: (iconInfo.tabId) ? iconInfo.tabId : this.tab.id,
    ...
    if (currentWindow) {
      ...
      if (isPageAction || isBrowserAction) {
        ...
        if (isPageAction) {
          ...
          if (img.classList.contains('enabled')) {
            ...
            if (extension.webContentsId) {
              this.$electron.remote.webContents.fromId(extension.webContentsId)
                .send('Monarc-page-action-clicked', { id: this.tab.id });
            }
          }
          ...
          if (extension.webContentsId) {
            this.$electron.remote.webContents.fromId(extension.webContentsId)
              .send('Monarc-browser-action-clicked', { id: this.tab.id });
          }
        }
      }
    }
  }
  removeMonarcExtension(extensionId: string): void {
    const ipc = this.$electron.ipcRenderer;
    ipc.send('remove-Monarc-extension', extensionId);
  }
  onContextmenu(extension: any): void {
    ...
    if (currentWindow) {
      ...
      menu.append(new MenuItem({
        ...
        click: () => {
          this.removeMonarcExtension(extension.extensionId);
        },
      }));
  ...
  mounted(): void {
    ...
    ipc.on(
      'Monarc-commands-execute-page-action',
    ...
    ipc.on(
      'Monarc-commands-execute-browser-action',
      (event, extensionId: string) => {
        const extension = this.$refs[`popover-${extensionId}`][0].referenceElm;
        extension.click();
      }
    ...
    ipc.on('add-Monarc-extension-result', (event, data: any): void => {
      if (data.result === 'OK') {
        (this.$parent as BrowserMainView).extensionService.update();
        ipc.send('Monarc-extension-added', data.name);
      }
    });
    ipc.on('remove-Monarc-extension-result', (event, data: any): void => {
      if (data.result === 'OK') {
        (this.$parent as BrowserMainView).extensionService.update();
      } else {
        // TODO: fix this
    ...
    ipc.on('remove-Monarc-extension', (event, extensionId) => {
      ipc.send('remove-Monarc-extension', extensionId);
    });
