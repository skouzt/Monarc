  ...
  el-row(type="flex", align="middle")
    ...
    el-col(:span="6", :offset="3")
      el-button(type="success",
                size="medium",
                @click="addMonarcExtension") {{ $t('about.extensionsPage.add') }}
  el-row
    el-col(:span="24")
      ul(class="extensions-list")
        li(v-for="extension in Object.keys(extensions)",
          ...
          el-button(:plain="true",
                    ...
                    size="mini",
                    icon="el-icon-close",
                    @click="removeMonarcExtension(extension)")
...
export default class Extensions extends Vue {
  ...
  addMonarcExtension(): void {
    window.ipcRenderer.once(
      'add-Monarc-extension-result',
      (event, data: any): void => {
        if (data.result === 'OK') {
    ...
    window.ipcRenderer.send('add-Monarc-extension');
  }
  removeMonarcExtension(extensionId: string): void {
    const id = this.findId(extensionId);
    window.ipcRenderer.once(
      'remove-Monarc-extension-result',
      (event, data: any): void => {
        if (data.result === 'OK') {
          window.location.reload();
        } else {
