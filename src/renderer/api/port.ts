...
class MessageSender {
  ...
  constructor(extensionId) {
    ...
    this.frameId = 0;
    this.id = extensionId;
    this.url = `Monarc-extension://${extensionId}`;
  }
...
class Port {
  ...
  _init(): void {
    ipcRenderer.on(`Monarc-runtime-port-${this.extensionId}-${this.name}`, (event, message) => {
      // https://developer.chrome.com/extensions/runtime#property-Port-onMessage
      this.onMessage.emit(message, this);
    });
    ipcRenderer.once(`Monarc-runtime-port-${this.extensionId}-${this.name}-disconnect`, () => {
    ...
    if (this.responseScriptType) {
      require('electron').remote.webContents.fromId(this.webContentsId)
        .send('Monarc-runtime-before-connect', this.extensionId, this.connectInfo, this.scriptType);
    } else {
  ...
  postMessage(message: any): void {
    require('electron').remote.webContents.fromId(this.webContentsId).send(
      `Monarc-runtime-port-${this.extensionId}-${this.name}`,
      message,
    );
  }
  ...
  _onDisconnect(): void {
    ...
    ipcRenderer.removeAllListeners(`Monarc-runtime-port-${this.extensionId}-${this.name}`);
    if (!this.otherEnd) {
      ipcRenderer.removeAllListeners(
        `Monarc-runtime-port-${this.extensionId}-${this.name}-disconnect`
      );
      require('electron').remote.webContents.fromId(this.webContentsId)
        .send(`Monarc-runtime-port-${this.extensionId}-${this.name}-disconnect`);
    }
    // https://developer.chrome.com/extensions/runtime#property-Port-onDisconnect
    this._Destroy();
  }
