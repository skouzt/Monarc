...
function findAndUpdateOrCreate(vueInstance: any, active: boolean, tabId: number, tabIndex?: number): Monarc.Store.TabObject {
  const dummyTabObject: Monarc.Store.TabObject = vueInstance.$store.getters.tabConfig.dummyTabObject;
  ...
  if (tabId === 0) {
    ...
    const tabObject: Monarc.Store.TabObject = vueInstance.getTabObject(tabIndex);
  ...
  const tabObject: Monarc.Store.TabObject = vueInstance.tabs.find(tab => (tab.id === tabId));
  if (tabObject === undefined) {
    return dummyTabObject;
  }
  if (active) {
...
export default (vueInstance: any) => {
  ...
  const tabs = {
    get: (tabId: number): Monarc.Store.TabObject => {
      const tab = findAndUpdateOrCreate(vueInstance, false, tabId);
      return tab;
    },
    getCurrent: (guestInstanceId: number): Monarc.Store.TabObject => {
    ...
    duplicate: (tabId: number): Monarc.Store.TabObject => {
      const tab = findAndUpdateOrCreate(vueInstance, false, tabId);
    ...
    query: (queryInfo: Monarc.API.CustomTabsQueryInfo): Monarc.Store.TabObject[] => {
      if (Object.keys(queryInfo).length === 0 || queryInfo.url === '<all_urls>') {
        return vueInstance.tabs;
      }

      const tempTabs: Monarc.Store.TabObject[] = [];
    ...
    update: (tabId: number, updateProperties: chrome.tabs.UpdateProperties = {}): Monarc.Store.TabObject => {
      const tab = findAndUpdateOrCreate(vueInstance, false, tabId);
      if (tab.windowId === vueInstance.windowId) {
    ...
    create: (createProperties: chrome.tabs.CreateProperties = {}): Monarc.Store.TabObject => {
    ...
    detectLanguage: (tabId: number, suffix: string, webContentsId: number): void => {
      ...
      if (tab.windowId === vueInstance.windowId) {
        vueInstance.getTab(getBuiltInTabIndex(vueInstance, tab.index)).$refs.webview.executeJavaScript(`
          ipcRenderer.send('Monarc-tabs-detect-language-result', navigator.language, ${suffix}, ${webContentsId});
        `);
      }
    },
    ...
    sendMessage: (tabId: number, message: any): void => {
      ...
      if (tab.windowId === vueInstance.windowId) {
        ...
        ).send('Monarc-tabs-send-message', message);
      }
    },
    ...
    sendMessage: (extensionId: string, message: any, external: boolean, webContentsId: number): void => {
      ...
      if (extension) {
        vueInstance.$electron.remote.webContents.fromId(extension.webContentsId)
          .send('Monarc-runtime-send-message', external, message, (webContents ? { url: webContents.getURL() } : { tab: findAndUpdateOrCreate(vueInstance, false, 0, tabIndex) }));
      }
    },
  ...
  const windows = {
    get: (windowId: number, getInfo: chrome.windows.GetInfo = {}): Monarc.Store.MonarcBrowserWindowProperty | undefined => {
      if (windowId === vueInstance.windowId) {
        const window: Monarc.Store.MonarcBrowserWindowProperty = Object.assign({}, vueInstance.window);
        if (getInfo.populate) {
    ...
    getCurrent: (getInfo: chrome.windows.GetInfo = {}, guestInstanceId: number): Monarc.Store.MonarcBrowserWindowProperty | undefined => {
      const webContents: Electron.WebContents = vueInstance.$electron.remote.getGuestWebContents(guestInstanceId);
      // https://github.com/electron/electron/blob/master/lib/browser/rpc-server.js#L133-L140
      if (!((webContents as any).type && (webContents as any).type === 'exception')) {
    ...
    getAll: (getInfo: chrome.windows.GetInfo = {}): Monarc.Store.MonarcBrowserWindowProperty => {
      const window: Monarc.Store.MonarcBrowserWindowProperty = Object.assign({}, vueInstance.window);
      if (getInfo.populate) {
        window.tabs = vueInstance.tabs;
      }
          ...
          vueInstance.getTab(getBuiltInTabIndex(vueInstance, tab.index)).$refs.webview.executeJavaScript(`
            ...
            for (i = 0; i < frames.length; i++) {
              if (frames[i].location.href.hashCode() === '${details.frameId}') {
                ...
                flag = false;
                ipcRenderer.send('Monarc-web-navigation-get-frame-result', frame, ${webContentsId});
                break;
              }
            }
            if (flag) {
              ...
              ipcRenderer.send('Monarc-web-navigation-get-frame-result', frame, ${suffix}, ${webContentsId});
            }
          `);
        }
      }
    ...
    getAllFrames: (details: chrome.webNavigation.GetAllFrameDetails, suffix: string, webContentsId: number): void => {
      ...
      if (tab.windowId === vueInstance.windowId) {
        ...
        vueInstance.getTab(getBuiltInTabIndex(vueInstance, tab.index)).$refs.webview.executeJavaScript(`
          ...
          ipcRenderer.send('Monarc-web-navigation-get-all-frames-result', framesArray, ${suffix}, ${webContentsId});
        `);
      }
    },
