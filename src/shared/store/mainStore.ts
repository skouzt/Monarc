...
function windowsOrdering(bumpWindowIdsBy: number, oneWindow = -1): Monarc.Store.MonarcBrowserWindowProperty[] {
  const newWindows: Monarc.Store.MonarcBrowserWindowProperty[] = [];
  let windowId: number = bumpWindowIdsBy === 0
    ? (1 + bumpWindowIdsBy)
    : (parseInt(Object.keys(windows)[0], 10) + bumpWindowIdsBy);
  if (oneWindow !== -1) {
    const id = oneWindow;
    const oldWindows: Monarc.Store.MonarcBrowserWindowProperty[] = store.getters.windows;
    const value: Monarc.Store.MonarcBrowserWindowProperty | undefined = oldWindows.find(window => window.id === id);
    if (value !== undefined) {
      const tmp: Monarc.Store.MonarcBrowserWindowProperty = Object.assign({}, value);
      tmp.id = windowId;
      newWindows.push(tmp);
    }
  } else {
...
const tabsMapping = (tabs: Monarc.Store.TabObject[], tabsOrder: number[]): number[] => {
...
function tabsOrdering(newStart: number, bumpWindowIdsBy: number, oneWindow = -1): Monarc.Store.TabsOrdering {
  let newTabId: number = newStart;
  let newTabs: Monarc.Store.TabObject[] = [];
  const newCurrentTabIndexes: number[] = [0];
  let windowId: number = bumpWindowIdsBy === 0
    ? 1
    : (parseInt(Object.keys(windows)[0], 10) + bumpWindowIdsBy);
  if (oneWindow !== -1) {
    const tmpTabs: Monarc.Store.TabObject[] = [];
    const id = oneWindow;
    const currentTabIndex: number = store.getters.currentTabIndexes[id];
    const oldTabs: Monarc.Store.TabObject[] =
    ...
    tmpTabs.forEach((tab) => {
      ...
      if (tab.url.startsWith('Monarc-extension:')) {
...
function collect(newStart: number, newTabs: Monarc.Store.TabObject[], newCurrentTabIndexes: number[], newWindows: Monarc.Store.MonarcBrowserWindowProperty[], downloads) {
  return {
...
function saveMonarcState(soft = true, bumpWindowIdsBy = 0): Promise<any> {
...
function bumpWindowIds(bumpWindowIdsBy: number): void {
  saveMonarcState(true, bumpWindowIdsBy).then((state) => {
    if (state && state.windows.length > 0) {
      ...
      state.windows = [];
      store.dispatch('setMonarcState', state);
    } else {
